...
class sha256:
	...
	def encrypt(self, p):
		m = [self.parse(x, 32) for x in self.parse(self.pad(p), 512)]
		
		for i, b in enumerate(m):
			W = b.copy()
			for t in range(16, 64):
				x = self.s1(W[t-2]) + W[t-7] + self.s0(W[t-15]) + W[t-16]
				W.append(x % 2**32)
			
			a, b, c, d, e, f, g, h = self.H
			for t in range(64):
				T1 = (h + self.S1(e) + self.ch(e, f, g) + self.K[t] + W[t]) % 2**32
				T2 = (self.S0(a) + self.maj(a, b, c)) % 2**32
				a, b, c, d, e, f, g, h = (T1 + T2) % 2**32, a, b, c, (d + T1) % 2**32, e, f, g
			
			n = [a, b, c, d, e, f, g, h]
			for t, h in enumerate(self.H):
				self.H[t] = (h + n[t]) % 2**32
